...
## PART 24: THE GHOST WRITER

```rust
// agentic-memory-core/src/v3/ghost_writer.rs

//! The Ghost Writer: Automatic sync to Claude Code's memory directory.
...
/// Runs in background, syncs context to Claude Code's memory directory
pub struct GhostWriter {
    /// Our V3 engine
    engine: Arc<MemoryEngineV3>,
    
    /// Claude Code's memory directory (auto-detected)
    claude_memory_dir: Option<PathBuf>,
    ...
    /// Last sync time
    last_sync: std::sync::Mutex<Option<chrono::DateTime<Utc>>>,
}

impl GhostWriter {
    /// Create and AUTO-START the ghost writer
    pub fn spawn(engine: Arc<MemoryEngineV3>) -> Arc<Self> {
        let writer = Arc::new(Self {
            engine,
            claude_memory_dir: Self::detect_claude_memory_dir(),
    ...
    /// Auto-detect Claude Code's memory directory
    fn detect_claude_memory_dir() -> Option<PathBuf> {
        // Strategy 1: Check ~/.claude/
        if let Some(home) = dirs::home_dir() {
            let claude_dir = home.join(".claude");
            if claude_dir.exists() {
                // Find the active project
                if let Some(project_dir) = Self::find_active_project(&claude_dir) {
                    let memory_dir = project_dir.join("memory");
                    if std::fs::create_dir_all(&memory_dir).is_ok() {
                        return Some(memory_dir);
                    }
                }
                
                // Fallback: use global memory dir
                let global_memory = claude_dir.join("memory");
                if std::fs::create_dir_all(&global_memory).is_ok() {
                    return Some(global_memory);
                }
            }
        }
        
        // Strategy 2: Check CLAUDE_PROJECT_DIR env var
        if let Ok(project_dir) = std::env::var("CLAUDE_PROJECT_DIR") {
            let memory_dir = PathBuf::from(project_dir).join("memory");
            if std::fs::create_dir_all(&memory_dir).is_ok() {
                return Some(memory_dir);
            }
        }
        
        // Strategy 3: Check CLAUDE_MEMORY_DIR env var (direct override)
        if let Ok(memory_dir) = std::env::var("CLAUDE_MEMORY_DIR") {
        ...
        // Strategy 4: Walk up from CWD to find .claude/
    ...
    /// Find active Claude project
    fn find_active_project(claude_dir: &Path) -> Option<PathBuf> {
        // Try reading Claude's state file
        let state_file = claude_dir.join("state.json");
        if state_file.exists() {
            if let Ok(content) = std::fs::read_to_string(&state_file) {
                if let Ok(state) = serde_json::from_str::<serde_json::Value>(&content) {
                    if let Some(project) = state.get("activeProject").and_then(|p| p.as_str()) {
                        let project_path = claude_dir.join("projects").join(project);
        ...
        if projects_dir.exists() {
            if let Ok(entries) = std::fs::read_dir(&projects_dir) {
                ...
                return entries.first().map(|e| e.path());
            }
        }
    ...
    pub fn sync_once(&self) {
        if let Some(ref dir) = self.claude_memory_dir {
            let context = self.engine.session_resume();
            
            // Write V3_CONTEXT.md
            let context_file = dir.join("V3_CONTEXT.md");
            let markdown = Self::format_as_claude_memory(&context);
            if std::fs::write(&context_file, &markdown).is_ok() {
                *self.last_sync.lock().unwrap() = Some(Utc::now());
            }
            
            // Merge into MEMORY.md if it exists
    ...
    pub fn claude_memory_dir(&self) -> Option<&Path> {
        self.claude_memory_dir.as_deref()
    }
    
    /// Format context as Claude-compatible markdown
    fn format_as_claude_memory(context: &SessionResumeResult) -> String {
        ...
        md.push_str("---\n");
        md.push_str("_This file is auto-generated by AgenticMemory V3. Do not edit manually._\n");
        
        md
    }
    
    /// Merge our context into existing MEMORY.md
    fn merge_into_memory_md(memory_file: &Path, context: &SessionResumeResult) {
        ...
        let start_marker = "<!-- AGENTIC_MEMORY_V3_START -->";
        let end_marker = "<!-- AGENTIC_MEMORY_V3_END -->";
        ...
        let _ = std::fs::write(memory_file, new_content);
    }
...
mod tests {
    ...
    fn test_format_as_claude_memory() {
        let context = SessionResumeResult {
            ...
            errors_resolved: vec![
                ("missing dep".to_string(), "added to Cargo.toml".to_string()),
            ],
...
## PART 25: THE MCP GREETING

```rust
// agentic-memory-mcp/src/greeting.rs

//! The MCP Greeting: Inject context at connection time.
//! Claude gets context BEFORE the first message.

use agentic_memory_core::v3::{MemoryEngineV3, SessionResumeResult};
use serde::{Deserialize, Serialize};

/// Server info structure for MCP protocol
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Generate server info with embedded session context
pub fn server_info_with_context(engine: &MemoryEngineV3) -> ServerInfo {
    let context = engine.session_resume();
    let context_str = format_greeting_context(&context);
    
    ServerInfo {
        name: "agentic-memory".to_string(),
        version: "3.0.0".to_string(),
        
        // THE KEY: Server description includes context
        // MCP clients display this to the AI at connection time
        description: Some(format!(
            "AgenticMemory V3 - Immortal cognitive memory for AI agents.\n\n\
            ═══════════════════════════════════════\n\
            SESSION CONTEXT (Auto-loaded)\n\
            ═══════════════════════════════════════\n\
            {}\n\
            ═══════════════════════════════════════\n\n\
            Tools available: memory_capture_*, memory_search_*, memory_retrieve, memory_session_resume\n\
            Resources: memory://session/context, memory://session/decisions, memory://session/files",
            context_str
        )),
...
/// Format context for greeting (compact but informative)
fn format_greeting_context(context: &SessionResumeResult) -> String {
    let mut parts = Vec::new();
    
    // Session ID
    parts.push(format!("Session: {}", &context.session_id[..8]));
    parts.push(format!("Blocks: {}", context.block_count));
    ...
    if !context.decisions.is_empty() {
        ...
        for (i, decision) in context.decisions.iter().take(5).enumerate() {
            let preview = if decision.len() > 80 {
                format!("{}...", &decision[..80])
            } else {
                decision.clone()
            };
        ...
        if context.decisions.len() > 5 {
            parts.push(format!("  ...and {} more", context.decisions.len() - 5));
        }
    }
    ...
    if !context.files_touched.is_empty() {
        ...
        for (path, op) in context.files_touched.iter().take(10) {
            parts.push(format!("  • {} ({})", path, op));
        }
        if context.files_touched.len() > 10 {
            parts.push(format!("  ...and {} more", context.files_touched.len() - 10));
        }
    }
    ...
    if !context.errors_resolved.is_empty() {
        parts.push(String::new());
        parts.push(format!("ERRORS RESOLVED: {}", context.errors_resolved.len()));
    }
    
    if parts.len() <= 2 {
        parts.push(String::new());
        parts.push("New session - no prior context".to_string());
    }
    
    parts.join("\n")
}

/// Generate a compact context string for resource descriptions
pub fn compact_context_string(context: &SessionResumeResult) -> String {
    let mut parts = Vec::new();
    
    if !context.decisions.is_empty() {
        parts.push(format!("{} decisions", context.decisions.len()));
    }
    if !context.files_touched.is_empty() {
        parts.push(format!("{} files", context.files_touched.len()));
    }
    if !context.errors_resolved.is_empty() {
        parts.push(format!("{} errors resolved", context.errors_resolved.len()));
    }
...
mod tests {
    ...
    #[test]
    fn test_server_info_includes_context() {
        // Would need mock engine for full test
        // This tests the format function
        let context = SessionResumeResult {
            session_id: "test-12345678".to_string(),
            block_count: 42,
            recent_messages: vec![],
            files_touched: vec![
                ("/src/main.rs".to_string(), "create".to_string()),
        ...
        let greeting = format_greeting_context(&context);
        
        assert!(greeting.contains("test-1234"));
        assert!(greeting.contains("Use Rust"));
        assert!(greeting.contains("/src/main.rs"));
    }
}
```
...
## PART 26: THE RESOURCE GATEWAY

```rust
// agentic-memory-mcp/src/resources.rs

//! The Resource Gateway: Expose context as MCP resources.
//! Claude can discover and read these automatically.
...
/// Resource handler for AgenticMemory V3
...
impl ResourceHandler {
    ...
    pub fn list_resources(&self) -> Vec<Resource> {
        let context = self.engine.session_resume();
        let compact = super::greeting::compact_context_string(&context);
        
        vec![
            Resource {
                uri: "memory://session/context".to_string(),
                name: "Session Context".to_string(),
                description: Some(format!(
                    "Full session context from AgenticMemory V3. Current: {}",
                    compact
                )),
                mime_type: Some("text/markdown".to_string()),
            },
            Resource {
                uri: "memory://session/decisions".to_string(),
                name: "Recent Decisions".to_string(),
                description: Some(format!(
            ...
            Resource {
                uri: "memory://session/files".to_string(),
                name: "Files Modified".to_string(),
                description: Some(format!(
            ...
            Resource {
                uri: "memory://session/errors".to_string(),
                name: "Errors Resolved".to_string(),
                description: Some(format!(
                    "{} errors resolved in recent sessions",
                    context.errors_resolved.len()
                )),
                mime_type: Some("text/markdown".to_string()),
            },
            Resource {
                uri: "memory://session/activity".to_string(),
                name: "Recent Activity".to_string(),
                description: Some("Recent messages and tool calls".to_string()),
                mime_type: Some("text/markdown".to_string()),
            },
            Resource {
                uri: "memory://stats".to_string(),
                name: "Memory Statistics".to_string(),
                description: Some("Storage and index statistics".to_string()),
    ...
    pub fn read_resource(&self, uri: &str) -> Option<ResourceContents> {
        match uri {
            "memory://session/context" => Some(self.read_context()),
            "memory://session/decisions" => Some(self.read_decisions()),
            "memory://session/files" => Some(self.read_files()),
            "memory://session/errors" => Some(self.read_errors()),
            "memory://session/activity" => Some(self.read_activity()),
            "memory://stats" => Some(self.read_stats()),
            _ => None,
        }
    }
    
    fn read_context(&self) -> ResourceContents {
        let context = self.engine.session_resume();
        let markdown = self.format_full_context(&context);
    ...
    fn read_decisions(&self) -> ResourceContents {
        let context = self.engine.session_resume();
        let mut md = String::new();
        ...
        if context.decisions.is_empty() {
            md.push_str("_No decisions recorded yet._\n");
        } else {
            for (i, decision) in context.decisions.iter().enumerate() {
    ...
    fn read_files(&self) -> ResourceContents {
        let context = self.engine.session_resume();
        let mut md = String::new();
        ...
        if context.files_touched.is_empty() {
            md.push_str("_No files modified yet._\n");
        } else {
            md.push_str("| Path | Operation |\n");
            md.push_str("|------|----------|\n");
            for (path, op) in &context.files_touched {
                md.push_str(&format!("| `{}` | {} |\n", path, op));
            }
        }
    ...
    fn read_errors(&self) -> ResourceContents {
        let context = self.engine.session_resume();
        let mut md = String::new();
        ...
        if context.errors_resolved.is_empty() {
            md.push_str("_No errors recorded._\n");
        } else {
            for (error, resolution) in &context.errors_resolved {
    ...
    fn read_activity(&self) -> ResourceContents {
        let context = self.engine.session_resume();
        let mut md = String::new();
        ...
        if context.recent_messages.is_empty() {
            ...
            for (role, msg) in &context.recent_messages {
                md.push_str(&format!("## [{}]\n\n", role));
                md.push_str(msg);
                md.push_str("\n\n---\n\n");
            }
    ...
    fn read_stats(&self) -> ResourceContents {
        ...
        ResourceContents {
            uri: "memory://stats".to_string(),
            mime_type: Some("application/json".to_string()),
            text: Some(serde_json::to_string_pretty(&json).unwrap()),
            blob: None,
        }
    }
    
    fn format_full_context(&self, context: &SessionResumeResult) -> String {
        let mut md = String::new();
        
        md.push_str("# AgenticMemory V3 Session Context\n\n");
        ...
        if context.recent_messages.is_empty() {
            ...
            for (role, msg) in context.recent_messages.iter().take(20) {
...
mod tests {
    ...
    fn test_list_resources() {
        ...
        assert_eq!(resources[0].uri, "memory://session/context");
    }
}
```
...
## PART 27: THE INSTALL HOOK

```rust
// agentic-memory-cli/src/install_hook.rs

//! The Install Hook: One-time Claude Code integration.
...
pub struct InstallResult {
    ...
    pub memory_dir_created: bool,
    pub instructions_added: bool,
    pub errors: Vec<String>,
}

/// Configure Claude Code to auto-load AgenticMemory context
pub fn install_claude_integration() -> InstallResult {
    let mut result = InstallResult {
        success: false,
        claude_dir: None,
        mcp_configured: false,
        memory_dir_created: false,
        instructions_added: false,
        errors: Vec::new(),
    };
    
    println!("🔧 Installing AgenticMemory V3 Claude Code integration...\n");
    
    // Step 1: Find Claude Code config
    match find_claude_config() {
        Ok(config_path) => {
            result.claude_dir = config_path.parent().map(|p| p.to_path_buf());
            println!("✓ Found Claude Code at: {:?}", result.claude_dir);
            
            // Step 2: Add MCP server
            match add_mcp_server_to_config(&config_path) {
                Ok(_) => {
                    result.mcp_configured = true;
                    println!("✓ Added AgenticMemory MCP server to config");
                }
                Err(e) => {
                    result.errors.push(format!("MCP config: {}", e));
                    println!("✗ Failed to add MCP server: {}", e);
                }
            }
            
            // Step 3: Create memory directory
            if let Some(ref claude_dir) = result.claude_dir {
                match create_memory_directory(claude_dir) {
                    Ok(_) => {
                        result.memory_dir_created = true;
                        println!("✓ Created memory directory");
                    }
                    Err(e) => {
                        result.errors.push(format!("Memory dir: {}", e));
                        println!("✗ Failed to create memory directory: {}", e);
                    }
                }
                
                // Step 4: Add instructions
                match add_context_instruction(claude_dir) {
                    Ok(_) => {
                        result.instructions_added = true;
                        println!("✓ Added integration instructions");
                    }
                    Err(e) => {
                        result.errors.push(format!("Instructions: {}", e));
                        println!("✗ Failed to add instructions: {}", e);
                    }
                }
            }
        }
        Err(e) => {
            result.errors.push(format!("Find Claude: {}", e));
            println!("✗ Could not find Claude Code: {}", e);
        }
    }
    
    // Summary
    println!();
    if result.mcp_configured && result.memory_dir_created {
        result.success = true;
        println!("════════════════════════════════════════════════════");
        println!("✅ AgenticMemory V3 integration installed!");
        println!("════════════════════════════════════════════════════");
        println!();
        println!("What happens now:");
        println!("  • AgenticMemory MCP server starts with Claude Code");
        println!("  • Session context is auto-synced to Claude's memory");
        println!("  • Context is available in every conversation");
        println!();
        println!("No further action needed. It just works.");
        println!();
    } else {
        println!("════════════════════════════════════════════════════");
        println!("⚠️  Partial installation. Some features may not work.");
        println!("════════════════════════════════════════════════════");
...
/// Find Claude Code's configuration file
fn find_claude_config() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let home = dirs::home_dir().ok_or("No home directory found")?;
    
    // Standard locations by platform
    let candidates = vec![
        // Linux / macOS (standard)
        home.join(".claude/settings/claude_desktop_config.json"),
    ...
    let default_config = default_dir.join("claude_desktop_config.json");
    std::fs::write(&default_config, "{}\n")?;
    
    Ok(default_config)
}

/// Add AgenticMemory MCP server to Claude's config
fn add_mcp_server_to_config(config_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Add our server (merge, don't overwrite)
    let mcp_servers = config["mcpServers"].as_object_mut()
        .ok_or("mcpServers is not an object")?;
    
    // Check if already configured
    if mcp_servers.contains_key("agentic-memory") {
        println!("  (AgenticMemory already in MCP config, updating...)");
    }
    ...
    mcp_servers.insert(
        ...
        serde_json::json!({
            "command": "amem-mcp",
            "args": ["--auto-context", "--ghost-writer"],
            "env": {
                "AGENTIC_MEMORY_AUTO_CAPTURE": "true"
            }
        }),
    );
...
fn create_memory_directory(claude_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    let memory_dir = claude_dir.join("memory");
    std::fs::create_dir_all(&memory_dir)?;
    
    // Create initial V3_CONTEXT.md
    let context_file = memory_dir.join("V3_CONTEXT.md");
    if !context_file.exists() {
        std::fs::write(
            &context_file,
            r#"# AgenticMemory V3 Context

> 🔄 This file is auto-updated by AgenticMemory's Ghost Writer.

_Waiting for first session to capture context..._

---

AgenticMemory V3 is installed and will capture:
- Every message
- Every tool call
- Every file operation
- Every decision

Context will appear here automatically.
"#,
        )?;
    }
...
/// Add integration instructions to Claude's project instructions
fn add_context_instruction(claude_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    let instructions_file = claude_dir.join("CLAUDE.md");
    let existing = std::fs::read_to_string(&instructions_file).unwrap_or_default();
    
    // Check if already present
    if existing.contains("AgenticMemory V3") {
        println!("  (Instructions already present, skipping)");
        return Ok(());
    }
    
    let addition = r#"

## AgenticMemory V3 Integration

AgenticMemory V3 is active with automatic context injection.

**What this means:**
- Your session context is automatically available at conversation start
- The file `memory/V3_CONTEXT.md` contains decisions, files touched, and history
- This updates every 5 seconds during active sessions
- No need to call memory tools manually for context

**Available when needed:**
- `memory_retrieve` - Smart context assembly for complex queries
- `memory_search_*` - Search by time, meaning, entity, or causation
- `memory_resurrect` - Restore full state at any past timestamp
- `memory://session/context` - MCP resource with full context

**Trust the context:**
The decisions, files, and history shown are cryptographically verified.
Nothing is lost. Nothing is modified. Everything is preserved.
"#;
    
    let new_content = format!("{}{}", existing.trim(), addition);
    std::fs::write(&instructions_file, new_content)?;
    
    Ok(())
}

/// Uninstall integration
pub fn uninstall_claude_integration() -> Result<(), Box<dyn std::error::Error>> {
    println!("🔧 Removing AgenticMemory V3 Claude Code integration...\n");
    ...
    if let Some(mcp_servers) = config["mcpServers"].as_object_mut() {
        mcp_servers.remove("agentic-memory");
        println!("✓ Removed MCP server from config");
    }
    
    let new_content = serde_json::to_string_pretty(&config)?;
    std::fs::write(&config_path, new_content)?;
    
    println!("\n✅ AgenticMemory integration removed.");
    println!("   Memory data in ~/.claude/memory/ is preserved.");
    
    Ok(())
}

/// Check installation status
pub fn check_installation_status() -> InstallResult {
    let mut result = InstallResult {
        success: false,
        claude_dir: None,
        mcp_configured: false,
    ...
    if let Ok(config_path) = find_claude_config() {
        result.claude_dir = config_path.parent().map(|p| p.to_path_buf());
        ...
        if let Ok(content) = std::fs::read_to_string(&config_path) {
            if let Ok(config) = serde_json::from_str::<serde_json::Value>(&content) {
                if let Some(servers) = config["mcpServers"].as_object() {
                    result.mcp_configured = servers.contains_key("agentic-memory");
                }
            }
        }
        ...
        if let Some(ref claude_dir) = result.claude_dir {
            let memory_dir = claude_dir.join("memory");
            result.memory_dir_created = memory_dir.exists();
            
            let instructions = claude_dir.join("CLAUDE.md");
            if let Ok(content) = std::fs::read_to_string(&instructions) {
                result.instructions_added = content.contains("AgenticMemory V3");
            }
        }
    }
    
    result.success = result.mcp_configured && result.memory_dir_created;
    result
}
...
mod tests {
    ...
    fn test_add_mcp_server() {
        ...
        add_mcp_server_to_config(&config_path).unwrap();
        
        let content = std::fs::read_to_string(&config_path).unwrap();
        let config: serde_json::Value = serde_json::from_str(&content).unwrap();
        
        assert!(config["mcpServers"]["agentic-memory"].is_object());
        assert_eq!(
            config["mcpServers"]["agentic-memory"]["command"],
            "amem-mcp"
        );
    }
    
    #[test]
    fn test_create_memory_directory() {
        let dir = TempDir::new().unwrap();
        
        create_memory_directory(dir.path()).unwrap();
        
        assert!(dir.path().join("memory").exists());
        assert!(dir.path().join("memory/V3_CONTEXT.md").exists());
        assert!(dir.path().join("memory/.gitignore").exists());
    }
}
```
...
## PART 28: CLI COMMANDS FOR INSTALLATION

```rust
// agentic-memory-cli/src/main.rs (additions)

use clap::{Parser, Subcommand};

#[derive(Parser)]
#[command(name = "amem")]
#[command(about = "AgenticMemory V3 - Immortal cognitive memory for AI agents")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
struct ServeArgs {
    ...
    /// Enable ghost writer (background sync)
    #[arg(long)]
    ghost_writer: bool,
    
    /// Port for HTTP transport (if not using stdio)
    #[arg(long)]
    port: Option<u16>,
}
...
fn main() {
    ...
    match cli.command {
        Commands::Install => {
            let result = install_hook::install_claude_integration();
            std::process::exit(if result.success { 0 } else { 1 });
        }
        
        Commands::Uninstall => {
            match install_hook::uninstall_claude_integration() {
        ...
        Commands::Status => {
            let status = install_hook::check_installation_status();
            println!("AgenticMemory V3 Installation Status:");
            println!("  Claude Code found: {}", status.claude_dir.is_some());
            println!("  MCP configured: {}", status.mcp_configured);
            println!("  Memory directory: {}", status.memory_dir_created);
            println!("  Instructions added: {}", status.instructions_added);
            println!();
            if status.success {
                println!("✅ Installation complete and working");
            } else {
                println!("⚠️  Installation incomplete");
                println!("   Run `amem install` to complete setup");
            }
        }
        ...
        Commands::V3(cmd) => {
            // Handle V3 subcommands
            handle_v3_command(cmd);
        }
    }
}
...
fn run_mcp_server(args: ServeArgs) {
    ...
    // Initialize engine
    let config = EngineConfig::default();
    let engine = Arc::new(MemoryEngineV3::open(config).expect("Failed to open memory engine"));
    
    // Start ghost writer if enabled
    let _ghost_writer = if args.ghost_writer {
        Some(GhostWriter::spawn(engine.clone()))
    } else {
        None
    };
    
    // Start MCP server
    println!("Starting AgenticMemory V3 MCP server...");
    if args.auto_context {
        println!("  Auto-context: enabled");
    }
    if args.ghost_writer {
        println!("  Ghost writer: enabled");
    }
    
    // Run MCP server (implementation depends on MCP SDK)
    // mcp_server::run(engine, args.port);
...
## PART 29: COMPLETE MODULE STRUCTURE (UPDATED)

```rust
// agentic-memory-core/src/v3/mod.rs (updated)
...
```rust
// agentic-memory-mcp/src/lib.rs (updated)
...
pub fn create_server(engine: Arc<MemoryEngineV3>, auto_context: bool) -> McpServer {
    let mut server = McpServer::new();
    
    // Set server info with context (MCP Greeting)
    if auto_context {
        server.set_info(greeting::server_info_with_context(&engine));
    }
    
    // Register resources (Resource Gateway)
    let resource_handler = resources::ResourceHandler::new(engine.clone());
    for resource in resource_handler.list_resources() {
        server.register_resource(resource);
    }
    
    // Register V3 tools
    v3_tools::register_v3_tools(&mut server, engine.clone());
    
    // Set up auto-capture middleware
    let auto_capture = auto_capture::AutoCaptureMiddleware::new(engine.clone());
    server.set_middleware(auto_capture);
...
## PART 30: COMPLETE FILE TREE (FINAL)
...
│           ├── recovery.rs             # WAL and crash recovery
│           ├── migration.rs            # V2 → V3 migration
│           ├── ghost_writer.rs         # ★ Background sync to Claude
│           ├── claude_hooks.rs         # Claude Code integration
│           └── tests.rs                # Unit tests
├── mcp/
│   ├── Cargo.toml
│   └── src/
│       ├── lib.rs                      # MCP server creation
│       ├── v3_tools.rs                 # V3 MCP tools (14+)
│       ├── greeting.rs                 # ★ MCP server info with context
│       ├── resources.rs                # ★ MCP resource gateway
│       └── auto_capture.rs             # Auto-capture middleware
├── cli/
│   ├── Cargo.toml
│   └── src/
│       ├── main.rs                     # CLI entry point
│       ├── install_hook.rs             # ★ One-time Claude setup
│       └── v3_commands.rs              # V3 CLI commands
└── README.md
```

**★ = New zero-friction components**
...
| Part | Content | Lines |
|------|---------|-------|
| 1-13 | Core V3 Implementation | ~3,500 |
| 14-18 | Infrastructure (compression, encryption, recovery, config) | ~1,000 |
| 19-23 | Execution Guide (prompts, phases, criteria) | ~700 |
| **24** | **Ghost Writer** (auto-sync to Claude) | ~400 |
| **25** | **MCP Greeting** (context at connection) | ~150 |
| **26** | **Resource Gateway** (MCP resources) | ~300 |
| **27** | **Install Hook** (one-time setup) | ~350 |
| **28** | **CLI Commands** (install/uninstall) | ~100 |
| **29-30** | Updated module structure | ~100 |

**Total: ~6,600+ lines of specification**
