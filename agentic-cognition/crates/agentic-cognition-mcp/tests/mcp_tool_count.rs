...
fn mcp_tool_descriptions_valid() {
    ...
    for tool in &tools {
        ...
        assert!(!tool.description.ends_with('.'), "Tool {} description ends with period", tool.name);
        // Descriptions must start with a capital letter (verb-first imperative)
        let first_char = tool.description.chars().next().unwrap();
        assert!(first_char.is_uppercase(), "Tool {} description must start with uppercase", tool.name);
    }
}
